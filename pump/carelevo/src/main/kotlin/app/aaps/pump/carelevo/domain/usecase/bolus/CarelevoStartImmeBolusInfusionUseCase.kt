...
class CarelevoStartImmeBolusInfusionUseCase @Inject constructor(
    ...
    /**
     * Persist a started immediate bolus (`mode=3` + imme-bolus infusion-info + `bolusActionSeq`) — extracted
     * from [execute]'s post-write branch so the Phase-2 new-BLE-stack path reuses the exact same writes.
     * [expectedTimeSeconds] is the pump-reported expected completion (seeds the synthetic progress loop).
