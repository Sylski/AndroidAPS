...
import app.aaps.pump.carelevo.domain.model.bt.CancelImmeBolusResultModel
...
class CarelevoCancelImmeBolusInfusionUseCase @Inject constructor(
     ...
     * the remaining infusion state) — extracted from [execute]'s post-write branch so the Phase-2 new-BLE-stack
     * path reuses the exact same writes. The pump-reported `infusedAmount` is recorded to the AAPS treatment DB
     * by the caller (not here). Returns false with no infusion/patch record or if any write fails.
     */
