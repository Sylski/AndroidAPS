...
import app.aaps.pump.carelevo.domain.model.bt.CancelTempBasalProgramResultModel
...
class CarelevoCancelTempBasalInfusionUseCase @Inject constructor(
    ...
    /**
     * Persist a cancelled temp basal (delete the temp-basal infusion-info + recompute the patch mode from the
     * remaining infusion state) — extracted from [execute]'s post-write branch so the Phase-2 new-BLE-stack
     * path reuses the exact same writes. Returns false with no infusion/patch record or if any write fails.
     */
