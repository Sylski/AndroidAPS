...
class CarelevoCancelExtendBolusInfusionUseCase @Inject constructor(
    ...
    /**
     * Persist a cancelled extended bolus (delete the extend-bolus infusion-info + recompute the patch mode
     * from the remaining infusion state) — extracted from [execute]'s post-write branch so the Phase-2
     * new-BLE-stack path reuses the exact same writes. Returns false with no infusion/patch record or if any
     * write fails. (The pump-reported `infusedAmount` is surfaced by the caller, not persisted here — matching
     * legacy, which does not reconcile it into the DB.)
     */
